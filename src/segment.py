...
def segment_modify_save(image,
    ...
    '''
    Given an image, segment it, modify each segment, and store modifications that produce correct prediction

    Arg(s):
        image : C x H x W np.array
            image to segment and modify
        image_name : str
            in format of '{class_name}-{train/val split}-{filename}'
        segmentation_methods_params : list[(str, dict)]
            list of tuples with name and parameters for each segmentation method
        seed : int or None
            optional seed to set for randomness (relevant for Gaussian noise)
        save_dir_root : str
    '''
    ...
    if len(success_name) > 0:
        informal_log("Saved modified images to {}".format(image_save_dir))
