...
SUPPORTED_DATASET_TYPES = ['2_Spurious_MNIST', '2_Rand_MNIST']
...
def assign_color(label: int,
    ...
    n_colors = int(dataset_type[0])
    correlation_type = dataset_type[1]

    if correlation_type == 'Rand':
        p = np.random.uniform()
        # TODO: left off here figure out how to know which bucket to draw color label from
def prepare_colored_mnist(root: str,
    ...
    assert is_valid_dataset_type(dataset_type)

    dataset_dir = os.path.join(root, dataset_type)
    ensure_dir(dataset_dir)

    if os.path.exists(os.path.join(dataset_dir, 'train.pt')) \
